"use client";

import { Award } from "lucide-react";
import TxButton from "@/components/TxButton";
import ClaimCooldown from "@/components/ClaimCooldown";

interface RecompensasTabProps {
  pendingUSDT?: number;
  pendingEUSD?: number;
  totalRewardsUSD?: number;
  withdrawFeeUSD?: number;
  withdrawNetUSD?: number;
  claimTxState?: string;
  onClaim?: () => Promise<void>;
}

export default function RecompensasTab({
  pendingUSDT = 0,
  pendingEUSD = 0,
  totalRewardsUSD = 0,
  withdrawFeeUSD = 0,
  withdrawNetUSD = 0,
  claimTxState = "idle",
  onClaim,
}: RecompensasTabProps) {
  return (
    <div className="max-w-2xl mx-auto bg-gradient-to-b from-[#D4AF37]/10 to-[#0d0d0f] border border-[#D4AF37]/20 rounded-3xl p-6 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-md font-bold uppercase tracking-wider flex items-center gap-2 text-[#D4AF37]">
          <Award size={20} /> Dividendos Acumulados
        </h2>
      </div>

      <div className="text-center py-8 border-y border-white/5 mb-6">
        <span className="text-xs text-white/40 uppercase tracking-widest block mb-2">
          Dividendos Prontos para Saque
        </span>

        <div className="space-y-2">
          <div className="text-2xl font-black text-emerald-400">
            {pendingUSDT.toFixed(15)} USDT
          </div>

          <div className="text-2xl font-bold text-blue-400">
            {pendingEUSD.toFixed(15)} eDollar
          </div>
        </div>
      </div>

      {totalRewardsUSD > 0 && (
        <div className="bg-black/40 border border-white/5 rounded-2xl p-5 mb-6 space-y-3">
          <div className="flex justify-between text-sm text-gray-300">
            <span>Total de Dividendos</span>
            <span className="font-bold text-white">
              {totalRewardsUSD.toFixed(9)} USD
            </span>
          </div>

          <div className="flex justify-between text-sm text-red-400">
            <span>Taxa de Processamento (1%)</span>
            <span>-{withdrawFeeUSD.toFixed(9)} USD</span>
          </div>

          <div className="h-px bg-white/10" />

          <div className="flex justify-between text-base font-black text-emerald-400">
            <span>Rendimento Líquido</span>
            <span>{withdrawNetUSD.toFixed(9)} USD</span>
          </div>
        </div>
      )}

      <TxButton
        state={claimTxState}
        idleText="SACAR DIVIDENDOS"
        className="w-full py-4 rounded-xl font-black uppercase text-xs tracking-widest bg-[#D4AF37] text-black hover:bg-white transition-all cursor-pointer"
        onClick={onClaim}
      />

      <div className="text-center text-[10px] text-white/30 mt-3">
        <ClaimCooldown />
      </div>
    </div>
  );
}