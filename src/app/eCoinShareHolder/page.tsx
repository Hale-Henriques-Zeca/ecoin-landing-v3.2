...
// O componente com o design da pagina principal
import MiningPage from "@/components/mining/eDollar/MiningMainPage/MiningPage";

type PoolType = "BNB" | "ECOIN" | "EDOLLAR" | null;
...
export default function MiningPortal() {
  ...
  // 🎛️ ESTADO DE SELEÇÃO DE POOL (BNB, ECOIN, EDOLLAR OU NULL)
  const [selectedPool, setSelectedPool] = useState<PoolType>(null);
  ...
  // Handler para acionar a eCoin Cloud Wallet (Native Wallet)
  const handleCloudWalletLogin = () => {
    // Insira aqui o disparo do modal ou método de autenticação da eCoin Cloud Wallet
    console.log(`Conectando à eCoin Cloud Wallet para o Pool: ${selectedPool}`);
  };
    ...
    <div className="min-h-screen bg-[#020617] text-white">
      ...
      {!isConnected && (
        <div className="relative flex flex-col items-center justify-center min-h-screen text-center px-6 py-20 overflow-hidden">
          ...
          {/* FUNDO DINÂMICO DE SHAREHOLDERS & CS */}
          <div
          ...
          <div className="relative z-10 max-w-4xl w-full space-y-8">
            ...
            >
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#D4AF37] text-[10px] font-black tracking-[0.3em] uppercase">
                <Zap size={14} className="animate-pulse" /> Distribuição de Dividendos & Capacidade CS em Milissegundos
              </div>
              <h1 className="text-3xl md:text-6xl font-black tracking-tighter leading-tight">
                Portal <span className="text-[#D4AF37]">Neural</span> <br />
            ...
            {/* ================= SELETOR DE POOLS (SWITCH DE ENTIDADES) ================= */}
            <div className="w-full max-w-3xl mx-auto pt-4">
              <h3 className="text-xs uppercase tracking-[0.3em] text-[#D4AF37] font-bold mb-4">
                Selecione o AI Trading Robot Pool
              </h3>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {/* POOL BNB */}
                <button
                  onClick={() => setSelectedPool("BNB")}
                  className={`relative p-5 rounded-2xl border text-left transition-all duration-300 flex flex-col justify-between ${
                    selectedPool === "BNB" 
                      ? "bg-[#D4AF37]/15 border-[#D4AF37] shadow-[0_0_25px_rgba(212,175,55,0.25)] scale-[1.02]" 
                ...
                {/* POOL EDOLLAR & USDT (DUAL POOL ATIVO) */}
                <button
                  onClick={() => setSelectedPool("EDOLLAR")}
                  className={`relative p-5 rounded-2xl border text-left transition-all duration-300 flex flex-col justify-between ${
                    selectedPool === "EDOLLAR" 
                      ? "bg-[#D4AF37]/15 border-[#D4AF37] shadow-[0_0_25px_rgba(212,175,55,0.25)] scale-[1.02]" 
                      : "bg-white/5 border-white/10 hover:border-white/30 hover:bg-white/[0.07]"
                  }`}
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {selectedPool === "EDOLLAR" && <CheckCircle2 size={18} className="text-[#D4AF37]" />}
                  </div>
                  <div>
                    <span className="text-[9px] text-cyan-400 font-bold tracking-widest uppercase block mb-1">Multi-Wallet Connect</span>
                    <h4 className="text-lg font-black text-white">eDollar & USDT</h4>
                    <p className="text-[11px] text-gray-400 mt-1">Dual Pool Ativo Existente</p>
                  </div>
                </button>
              </div>
            </div>

            {/* ================= MÉTRICAS E BOTÃO DE LOGIN (APARECEM APENAS APÓS SELEÇÃO) ================= */}
            {selectedPool && (
              <motion.div 
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                {/* 📊 DASHBOARD ECOSYSTEM METRICS */}
                <div className="max-w-5xl mx-auto border-t border-white/10 pt-8">
                  <h2 className="text-2xl md:text-3xl font-black mb-8 text-center tracking-tight">
                    📊 Métricas do <span className="text-[#D4AF37]">Ecossistema</span> ({selectedPool === "EDOLLAR" ? "eDollar & USDT" : selectedPool})
                  </h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                      ...
                      { 
                        label: "Staking Global", 
                        value: stats.totalStaked, 
                        icon: ShieldCheck,
                        color: "green" as const
                      },
                      { 
                        label: "Shareholders Ativos", 
                    ...
                    >
                      <div className="flex flex-col text-left">
                        <span className="text-[10px] text-white/40 uppercase font-bold tracking-widest">
                          Entenda a origem dos dividendos e do Selo de Compromisso (CS)
                        </span>
                        <span className="text-xs text-[#F5F5F5] font-semibold">
                          Guia Completo de Shareholders & Dividendos
                        </span>
                      </div>
                ...
                {/* ÁREA DE CONEXÃO ESPECÍFICA DO POOL */}
                <div className="flex flex-col items-center gap-6">
                  {/* Para BNB ou ECOIN -> Botão eCoin Cloud Wallet (Native Wallet) */}
                  {(selectedPool === "BNB" || selectedPool === "ECOIN") && (
                    <button
                    ...
                    >
                      ...
                      <span className="tracking-[0.2em] uppercase text-sm">
                        Login via eCoin Cloud Wallet
                      </span>
                    </button>
                  )}

                  {/* Para EDOLLAR -> ConnectButton RainbowKit (Multi-Wallet) */}
                  {selectedPool === "EDOLLAR" && (
                  ...
                  <div className="grid grid-cols-1 gap-3 w-full max-w-md">
                    <div className="bg-white/5 border border-white/10 p-5 rounded-3xl backdrop-blur-md text-left">
                      <p className="text-[#D4AF37] font-bold text-xs mb-3 flex items-center gap-2 uppercase tracking-widest">
                        <ShieldCheck size={16} /> Protocolo Anti-Hack & Custódia Direta
                      </p>
                      <ul className="text-[11px] text-gray-400 space-y-2.5">
                        ...
                        <li className="flex items-center gap-2">✅ Dividendos creditados diretamente na sua Wallet</li>
                        <li className="flex items-center gap-2 font-bold text-white">✅ Ativos protegidos pela BNB Chain Network</li>
                      </ul>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-8 border-t border-white/5">
                  <p className="text-xs text-gray-500 italic">
                    Sem carteira? Proteja sua posição de Shareholder. Prepare seu cofre digital aqui:
                  </p>
                  ...
                  <p className="text-[9px] text-gray-600 font-mono uppercase tracking-[0.3em]">
                    2026: Independência financeira permanente (Rendimentos e CS em tempo real) 🚀
                  </p>
                </div>
              </motion.div>
            )}
      ...
      {/* ================= ESTADO: CONECTADO (TERMINAL DE SHAREHOLDERS) ================= */}
        ...
        >
          ...
          <div className="max-w-[1400px] mx-auto px-6 mb-8 flex justify-center">
            <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-emerald-500/5 border border-emerald-500/20 text-emerald-400 text-[10px] font-mono">
              ...
              SHAREHOLDER ATIVO: {address?.slice(0, 10)}...{address?.slice(-8)}
            </div>
          </div>
