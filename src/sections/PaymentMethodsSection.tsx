...
export default function PaymentMethodsSection() {
  ...
  const comingSoon = [
    ...
    "Crypto ATM, Vaults Banking, POS e QR Payments (pagamentos presenciais)",
    "Cartões físicas e virtuais E-Pay",
  ...
  {/* Glow E-Pay (ouro-vermelho + oceano azul) */}
  <div className="pointer-events-none absolute -top-24 left-1/2 h-72 w-72 -translate-x-1/2 rounded-full bg-gradient-to-r from-[#C9A24D]/20 via-[#B84A3A]/20 to-[#0A2A4F]/20 blur-3xl" />
  ...
  <div className="max-w-6xl mx-auto relative">
    ...
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ duration: 0.6 }}
      className="mb-12 rounded-2xl border border-[#C9A24D]/30 bg-gradient-to-br from-[#B84A3A]/10 via-black/40 to-[#0A2A4F]/20 p-6"
    >
      <h3 className="text-2xl font-bold text-[#C9A24D] flex items-center gap-2">
        <FaCheckCircle className="text-[#B84A3A]" />
      ...
      <div className="mt-6 flex flex-wrap gap-4">
        <button className="px-6 py-3 rounded-xl font-semibold bg-gradient-to-r from-[#C9A24D] via-[#B84A3A] to-[#0A2A4F] text-black hover:opacity-90 transition">
          Comprar via E-Pay
        </button>

        <button className="px-6 py-3 rounded-xl font-semibold border border-[#0A2A4F]/50 text-white hover:bg-[#0A2A4F]/20 transition">
          Ver Guia por País
        </button>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-3 text-gray-300 text-sm mt-6">
        ...
        <p className="mt-8 text-xs text-center text-gray-500 max-w-4xl mx-auto">
          ...
          Conversões podem ocorrer via parceiros regulados ou plataformas P2P.
        </p>
      </div>
    </section>
  );
